...
KISSY.add(function(S) {
	...
	var DomBase;
	typeof window.localStorage !== 'undefined' ? S.use('offiline/localstorage', function(S, Dom) {
		DomBase = Dom;
	}) : S.UA.ie < 8 ? S.use('offiline/ie-offline', function(S, Dom) {
		DomBase = Dom;
	...
	S.extend(Offline, S.Base, /** @lends Offline.prototype*/ {
		...
		setItem: function(key, value, deadline) {
			if(!S.isString(key) || !S.isString(value) || key === '') {
			...
			return DomBase.setItem(key, value, deadline);
		},
		/**
		 * 根据字段获取本地存储的值
